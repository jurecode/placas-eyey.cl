...
const REPO_POR_DEFECTO = 'jurecode/placas-eyey.cl';
const ARCHIVO_ZIP = 'placas-eyey.zip';
...
function github(string $ruta, ?string $guardarEn = null, bool $binario = false): array {
    /* Este repositorio es público, así que el token no hace falta. Se manda
     * igual si está cargado: con token GitHub deja hacer muchas más consultas
     * por hora, y si algún día el repo pasa a privado sigue andando. */
    ...
    if ($r['codigo'] === 401 || $r['codigo'] === 403) {
        throw new RuntimeException('GitHub rechazó la consulta'
            . ($token === '' ? '. Si el repositorio pasó a privado, cargá un token abajo.'
                             : ' (¿el token tiene permiso de lectura sobre el repo?)'));
    }
